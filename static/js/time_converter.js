// This file contains JavaScript logic specifically for the Time converter.
// It relies on common functions and variables defined in script.js (which must be loaded first).

document.addEventListener('DOMContentLoaded', () => {
    // Get elements relevant to Time converter from the DOM.
    ...
    // List of existing specific time conversion pages (filenames without .html)
    const existingSpecificTimePages = [
        'minutes_to_hours', 'hours_to_minutes',
        'hours_to_days', 'days_to_hours',
        'seconds_to_days', 'days_to_seconds',
        'minutes_to_days', 'days_to_minutes',
        'days_to_months', 'months_to_days',
        'days_to_years', 'years_to_days',
    ...
    // Populates 'From' and 'To' unit dropdowns for the time converter.
    function populateTimeUnitDropdowns(fixedFromUnit = null, fixedToUnit = null) {
        ...
        if (fixedFromUnit && fixedToUnit) {
            ...
            // For the main time converter page, show and populate dropdowns.
            fromUnitSelect.style.display = '';
            toUnitSelect.style.display = '';

            const timeUnits = units.time || [];
            ...
            if (timeUnits.length > 0) {
                fromUnitSelect.value = 'Hour'; 
                if (timeUnits.includes('Minute')) {
                    toUnitSelect.value = 'Minute';
                } else if (timeUnits.length > 1) {
                    toUnitSelect.value = timeUnits[1];
                } else {
                    toUnitSelect.value = timeUnits[0];
                }
            }
        }
    ...
    // Initial page setup for Time converter
    function initializeTimeConverterPage() {
        const { converterType, fromUnit, toUnit } = getConverterInfoFromUrl();

        if (converterType !== 'time') {
            return; // Exit if not on a time converter page
        }
        ...
        if (mainConverterSection) {
            ...
            if (fromUnit && toUnit) {
                ...
                if (heading) {
                    heading.textContent = 'Time Converter';
                }
                if (description) {
                    description.textContent = 'Convert between various time units like seconds, minutes, hours, and days.';
                }
            }
        }
        
        populateTimeUnitDropdowns(fromUnit, toUnit);
    }
    ...
    if (searchUnitsButton && searchFromUnitInput && searchToUnitInput) {
        searchUnitsButton.addEventListener('click', () => {
            ...
            const matchedFromUnit = findBestMatch(fromSearchText, timeUnits);
            const matchedToUnit = findBestMatch(toSearchText, timeUnits);

            if (matchedFromUnit && matchedToUnit) {
                const fromUrlPart = matchedFromUnit.toLowerCase().replace(/ \(.+\)/, '').replace(/ /g, '_');
                const toUrlPart = matchedToUnit.toLowerCase().replace(/ \(.+\)/, '').replace(/ /g, '_');
                
                const potentialPageName = `${fromUrlPart}_to_${toUrlPart}`;

                if (existingSpecificTimePages.includes(potentialPageName)) {
                    window.location.href = `/time/${potentialPageName}.html`;
                } else {
                    if (fromUnitSelect && toUnitSelect) {
                        ...
                        searchUnitsResultMessage.textContent = "Cannot set units on this page. Please use the main converter on the Time Home page.";
                        searchUnitsResultMessage.classList.add('error-message');
                    }
                }
            } else {
                ...
                errorMessage += ` in Time units. Please try full unit names (e.g., Second, Hour).`;
    ...
    // Initialize Time converter functionality when the DOM is fully loaded
    initializeTimeConverterPage();
});