// This file contains JavaScript logic specifically for the Pressure converter.
// It relies on common functions and variables defined in script.js (which must be loaded first).

document.addEventListener('DOMContentLoaded', () => {
    // Get elements relevant to Pressure converter from the DOM.
    ...
    // List of existing specific pressure conversion pages (filenames without .html)
    const existingSpecificPressurePages = [
    ...
    // Populates 'From' and 'To' unit dropdowns for the pressure converter.
    function populatePressureUnitDropdowns(fixedFromUnit = null, fixedToUnit = null) {
        ...
        if (fixedFromUnit && fixedToUnit) {
            ...
            // For the main pressure converter page, show and populate dropdowns.
            ...
            if (pressureUnits.length > 0) {
                fromUnitSelect.value = 'Bar'; 
                if (pressureUnits.includes('PSI')) {
                    toUnitSelect.value = 'PSI';
                } else if (pressureUnits.length > 1) {
                    toUnitSelect.value = pressureUnits[1];
                } else {
                    toUnitSelect.value = pressureUnits[0];
                }
            }
        }
    ...
    // Initial page setup for Pressure converter
    function initializePressureConverterPage() {
        const { converterType, fromUnit, toUnit } = getConverterInfoFromUrl();

        if (converterType !== 'pressure') {
            return; // Exit if not on a pressure converter page
        }
        ...
        if (mainConverterSection) {
            ...
            if (fromUnit && toUnit) {
                ...
                if (heading) {
                    heading.textContent = 'Pressure Converter';
                }
                if (description) {
                    description.textContent = 'Convert various pressure units like Pascals, PSI, Bar, and Atmosphere.';
                }
            }
        }
        
        populatePressureUnitDropdowns(fromUnit, toUnit);
    }
    ...
    // Search Units Logic for Pressure
    if (searchUnitsButton && searchFromUnitInput && searchToUnitInput) {
        searchUnitsButton.addEventListener('click', () => {
            ...
            const matchedFromUnit = findBestMatch(fromSearchText, pressureUnits);
            const matchedToUnit = findBestMatch(toSearchText, pressureUnits);
            ...
            if (matchedFromUnit && matchedToUnit) {
                ...
                if (existingSpecificPressurePages.includes(potentialPageName)) {
                    window.location.href = `/pressure/${potentialPageName}.html`;
                } else {
                    if (fromUnitSelect && toUnitSelect) {
                        ...
                        searchUnitsResultMessage.textContent = "Cannot set units on this page. Please use the main converter on the Pressure Home page.";
                        searchUnitsResultMessage.classList.add('error-message');
                    }
                }
            } else {
                ...
                errorMessage += ` in Pressure units. Please try full unit names (e.g., Pascal, Bar).`;
    ...
    // Initialize Pressure converter functionality when the DOM is fully loaded
    initializePressureConverterPage();
});