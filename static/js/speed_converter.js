// This file contains JavaScript logic specifically for the Speed converter.
// It relies on common functions and variables defined in script.js (which must be loaded first).

document.addEventListener('DOMContentLoaded', () => {
    // Get elements relevant to Speed converter from the DOM.
    ...
    // List of existing specific speed conversion pages (filenames without .html)
    const existingSpecificSpeedPages = [
        'mph_to_kph', 'kph_to_mph',
        'meters_per_second_to_miles_per_hour', 'miles_per_hour_to_meters_per_second',
        'knot_to_mph', 'mph_to_knot',
        'feet_per_second_to_mph', 'mph_to_feet_per_second',
        'meters_per_second_to_kph', 'kph_to_meters_per_second'
    ];
    ...
    // Populates 'From' and 'To' unit dropdowns for the speed converter.
    function populateSpeedUnitDropdowns(fixedFromUnit = null, fixedToUnit = null) {
        ...
        if (fixedFromUnit && fixedToUnit) {
            ...
            // For the main speed converter page, show and populate dropdowns.
            ...
            if (speedUnits.length > 0) {
                fromUnitSelect.value = 'Kilometer per hour'; 
                if (speedUnits.includes('Mile per hour')) {
                    toUnitSelect.value = 'Mile per hour';
                } else if (speedUnits.length > 1) {
                    toUnitSelect.value = speedUnits[1];
                } else {
                    toUnitSelect.value = speedUnits[0];
                }
            }
        }
    ...
    // Initial page setup for Speed converter
    function initializeSpeedConverterPage() {
        const { converterType, fromUnit, toUnit } = getConverterInfoFromUrl();

        if (converterType !== 'speed') {
            return; // Exit if not on a speed converter page
        }
        ...
        if (mainConverterSection) {
            ...
            if (fromUnit && toUnit) {
                ...
                if (heading) {
                    heading.textContent = 'Speed Converter';
                }
                if (description) {
                    description.textContent = 'Convert between various speed units like km/h, mph, m/s, and knots.';
                }
            }
        }
        
        populateSpeedUnitDropdowns(fromUnit, toUnit);
    }
    ...
    if (searchUnitsButton && searchFromUnitInput && searchToUnitInput) {
        searchUnitsButton.addEventListener('click', () => {
            ...
            const matchedFromUnit = findBestMatch(fromSearchText, speedUnits);
            const matchedToUnit = findBestMatch(toSearchText, speedUnits);
            ...
            if (matchedFromUnit && matchedToUnit) {
                ...
                if (existingSpecificSpeedPages.includes(potentialPageName)) {
                    window.location.href = `/speed/${potentialPageName}.html`;
                } else {
                    if (fromUnitSelect && toUnitSelect) {
                        ...
                        searchUnitsResultMessage.textContent = "Cannot set units on this page. Please use the main converter on the Speed Home page.";
                        searchUnitsResultMessage.classList.add('error-message');
                    }
                }
            } else {
                ...
                errorMessage += ` in Speed units. Please try full unit names (e.g., Kilometer per hour, Mile per hour).`;
    ...
    // Initialize Speed converter functionality when the DOM is fully loaded
    initializeSpeedConverterPage();
});